...
class YoutubeMp3Downloader extends EventEmitter {
  constructor(options) {
    ...
    this.youtubeVideoQuality =
      options && options.youtubeVideoQuality
        ...
        : "highestaudio";
    this.outputFormat = options && options.outputFormat ? options.outputFormat : "mp4"
    this.outputPath =
  ...
  async performDownload(task, callback) {
    ...
    const fileName = task.fileName
      ? self.outputPath + "/" + sanitize(task.fileName) + `.${self.outputFormat}`
      : self.outputPath + "/" + (videoTitle || info.videoId) + `.${self.outputFormat}`;
